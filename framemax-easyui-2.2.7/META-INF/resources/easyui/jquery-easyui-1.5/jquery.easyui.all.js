...
//				}
				var checked = !tt.is(':checked');
				tt._propAttr('checked', checked);
				if(!checked){
...
//				var tr = '<tr id="' + rowId + '" class="datagrid-row" datagrid-row-index="' + index + '">' + this.renderRow.call(this, target, fields, frozen, index, row) + '</tr>';
	...
	$.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
		...
		url: null,
		data: null,
		loadMsg: 'Processing data, please wait...',
		emptyMsg: '',
		rownumbers: false,
		singleSelect: false,
		...
		default:
			if (opts.editable){
				...
				state.timer = setTimeout(function(){
					...
					var v = t.combo('getValue');
					var invVal = (q == '' && !!v);
					if (state.previousText != q || invVal){
...
//	}
	...
	function initValue(target){
		...
		if (opts.multiple){
			$(target).combo('setValues', opts.value ? opts.value : []);
		} else {
			$(target).combo('setValue', opts.value);
...
//				if(state.options.limitToGrid){
//					var text = [];
//					$.each(state.options.selectedRows,function(i,row){
//						text.push(row[state.options.textField]);
//					});
//					$(this).combo('setText',text.join(state.options.separator));
//				}
//			},			
			onShowPanel: function(){
				setValues(this, $(this).combogrid('getValues'), true);
				var state = $.data(this,'combogrid');
				state.grid.data('datagrid').selectedRows = state.options.selectedRows.concat();
...
(function($){
	...
	function setValue(target, value, remainText){
		...
		date = date || opts.parser.call(target, value);
		if(date){
